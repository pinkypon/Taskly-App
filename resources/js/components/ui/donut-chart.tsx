            ...
            <div className="flex-1 space-y-2">
                {Object.entries(statusCounts).map(([status, count], index) => {
                    ...
                    return (
                        <div
                            key={status}
                            className={`flex items-center justify-between rounded-md px-2 py-1.5 text-xs transition-all duration-500 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-2 opacity-0'
                                } ${colors.bg}`}
